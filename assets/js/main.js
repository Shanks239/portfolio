...
const disciplines = {
  privacy: {
    title: '01 / PRIVACY & CRYPTOGRAPHY',
    description: 'What can a system prove without revealing what it knows?',
  },
  ai: {
    title: '02 / ARTIFICIAL INTELLIGENCE',
    description: 'Exploring what intelligent guidance can feel like in someone’s hands.',
  },
  creative: {
    title: '03 / CREATIVE COMPUTING',
    description: 'Code as a medium for feeling, play, and small moments of connection.',
  },
  onchain: {
    title: '04 / ON-CHAIN APPLICATIONS',
    description: 'Shared rules, private data, and interactions recorded on-chain.',
  },
  communication: {
    title: '05 / COMMUNICATION SYSTEMS',
    description: 'Extending an existing intercom foundation into a contest build.',
  },
};
document.querySelectorAll('[data-discipline]').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    projects
      .filter((p) => p.category.split(' ').includes(key))
      .forEach((project) => {
        const b = document.createElement('button');
        b.className = 'discipline-project';
        b.dataset.project = project.id;
...
if ('IntersectionObserver' in window)
  new IntersectionObserver(([entry]) => {
    ...
    updateMotion();
  }).observe(document.querySelector('.observatory'));
updateMotion();
...
function renderWriting() {
  const matching = writingRows.filter(
    (row) => writingFilter === 'all' || row.dataset.writingKind === writingFilter,
  );
  writingRows.forEach((row) => {
    row.hidden = true;
  });
  matching.slice(0, allWritingShown ? matching.length : 4).forEach((row) => {
    row.hidden = false;
  });
  showWriting.hidden = matching.length <= 4;
  showWriting.setAttribute('aria-expanded', String(allWritingShown));
  showWriting.textContent = allWritingShown
    ? 'Show fewer notes ↑'
    : `Show all ${matching.length} notes ↓`;
  document.getElementById('writing-status').textContent =
    `${Math.min(allWritingShown ? matching.length : 4, matching.length)} of ${matching.length} notes shown.`;
}
document.querySelectorAll('[data-writing-filter]').forEach((button) =>
  button.addEventListener('click', () => {
    writingFilter = button.dataset.writingFilter;
    allWritingShown = false;
    document
      .querySelectorAll('[data-writing-filter]')
      .forEach((b) => b.setAttribute('aria-pressed', String(b === button)));
    renderWriting();
  }),
);
showWriting.addEventListener('click', () => {
  ...
  allWritingShown = !allWritingShown;
  renderWriting();
...
singularityButton.addEventListener('click', () => {
  ...
  singularityButton.querySelector('.singularity-label').textContent = active
    ? 'RETURN TO ORBIT ↗'
    : 'ENTER SINGULARITY ↗';
